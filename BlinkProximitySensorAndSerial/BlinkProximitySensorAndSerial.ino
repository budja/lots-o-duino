...
void loop() {
  ...
  if ( !apds.readProximity(proximityData) ) {
    ...
    if (proximityData != oldProximityData)
    {
      Serial.print("Proximity value changed. New value is ");
      Serial.println(proximityData, DEC);
      oldProximityData = proximityData;
    }
  }

  // turn the ledPin on
  digitalWrite(ledPin, HIGH);  
  // stop the program for <proximityData * 4> milliseconds:
  delay(1024 - (proximityData << 2));
  // turn the ledPin off
